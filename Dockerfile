...
# Ensure gRPC C++ and core gRPC libraries built/installed in the build stage
# are available at runtime. This copies common libgrpc and libgrpc++ shared
# objects from the build stage into the runtime image so the `bessd` binary
# can find the exact ABI it was linked against.
COPY --from=bess-build /usr/lib/x86_64-linux-gnu/libgrpc*.so* /lib/x86_64-linux-gnu/
COPY --from=bess-build /usr/lib/x86_64-linux-gnu/libgrpc++*.so* /lib/x86_64-linux-gnu/
COPY --from=bess-build /usr/local/lib/x86_64-linux-gnu/libgrpc*.so* /lib/x86_64-linux-gnu/

# Also copy gRPC's dependency libraries (absl, gpr, cares, protobuf, re2, etc.)
COPY --from=bess-build /lib/x86_64-linux-gnu/libabsl*.so* /lib/x86_64-linux-gnu/
COPY --from=bess-build /lib/x86_64-linux-gnu/libgpr*.so* /lib/x86_64-linux-gnu/
COPY --from=bess-build /lib/x86_64-linux-gnu/libcares*.so* /lib/x86_64-linux-gnu/
COPY --from=bess-build /lib/x86_64-linux-gnu/libgrpc_plugin_support*.so* /lib/x86_64-linux-gnu/
COPY --from=bess-build /lib/x86_64-linux-gnu/libgrpcpp_channelz*.so* /lib/x86_64-linux-gnu/
COPY --from=bess-build /lib/x86_64-linux-gnu/libgrpc_unsecure*.so* /lib/x86_64-linux-gnu/
COPY --from=bess-build /lib/x86_64-linux-gnu/libprotobuf*.so* /lib/x86_64-linux-gnu/
COPY --from=bess-build /lib/x86_64-linux-gnu/libre2*.so* /lib/x86_64-linux-gnu/

# Also attempt copies from /usr/local if present (some build artifacts land there)
COPY --from=bess-build /usr/local/lib/x86_64-linux-gnu/libabsl*.so* /lib/x86_64-linux-gnu/
COPY --from=bess-build /usr/local/lib/x86_64-linux-gnu/libgpr*.so* /lib/x86_64-linux-gnu/
COPY --from=bess-build /usr/local/lib/x86_64-linux-gnu/libprotobuf*.so* /lib/x86_64-linux-gnu/
