...
* Proof of XSKMAP entry with bpftool

After running the userspace program and binding the AF_XDP socket, you can verify the XSKMAP entry using:
...
This shows that the XSKMAP contains an entry for queue 0 (key=0) with a valid socket FD (value=42).

* Deterministic UDP/4242 forwarding test and kernel proof

1. Send UDP packets to port 4242 on the test interface:

#+BEGIN_SRC sh
sudo nping --udp -p 4242 --data "hello-xdp" <test-interface-ip>
#+END_SRC

2. Userspace forwards received packets to the peer interface (e.g., eth1):

Ensure your userspace program sends packets out via the peer interface after receiving them from AF_XDP.

3. Capture on the peer interface to prove kernel delivery:

#+BEGIN_SRC sh
sudo tcpdump -i eth1 udp port 4242 -vv -c 5
#+END_SRC

Example tcpdump output:

#+BEGIN_SRC sh
17:14:31.123456 IP test-host.12345 > peer-host.4242: UDP, length 10
17:14:31.123457 IP test-host.12345 > peer-host.4242: UDP, length 10
...
#+END_SRC

This demonstrates that UDP/4242 packets are received by the kernel stack on the peer interface after userspace forwarding.

* Fully reproducible AF_XDP test setup
...
#+BEGIN_SRC sh
# Activate Python venv and install scapy (if needed)
python3 -m venv venv
source venv/bin/activate
pip install --upgrade pip scapy

# Run the forwarder (example)
